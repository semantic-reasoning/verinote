...
EXTRACTION_SYSTEM = (
    ...
    "the related entity or value. Normalize subjects, relations, and objects to "
    "concise source-language fact terms instead of copying whole source phrases. "
    "Preserve the source document's language, script, and named-entity spelling; "
    "do not translate, romanize, summarize, or replace source terms with labels "
    "from another language. If a fact is normalized, compacted, corrected, or "
    "derived from layout, put the exact original supporting phrase in note. "
    "Do not infer a relationship merely because two entities appear in the same "
    "chunk; the relation must be explicit in the same sentence, table row, bullet, "
    "or layout record. For numeric, percentage, count, date, or money facts, the "
    "subject must appear in that same local evidence record; do not reuse a subject "
    "from a previous or overlapping chunk. For key-value or label-value text, do "
    "not emit generic predicates like `값`; use relation `value` when no clearer "
    "owner/predicate is available. Do not use `is_a` unless the object is a class, "
    "category, or type of the subject. Do not use sentence endings such as `입니다` "
    "as objects, and do not emit question or judgment predicates ending in `여부`. "
